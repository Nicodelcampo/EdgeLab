...
namespace NinjaTrader.NinjaScript.Indicators
{
    public class HFTClusterZonesNQDx : Indicator
    {
        ...
        private void EvaluarHaloClusters(int newIdx)
        {
            ...
            for (int c = 0; c < islands.Count; c++)
            {
                ...
                for (int ex = clusters.Count - 1; ex >= 0; ex--)
                {
                    ...
                    if (prev.State == HFTClusterState.Depleted || prev.State == HFTClusterState.Invalidated || prev.State == HFTClusterState.Expired)
                        continue;
        ...
        private void VerificarExpiracionClusters()
        {
            ...
            int limitBars = SoloMuertePorBarras ? BarrasExtensionMuerte : MaxClusterAgeBars;
            for (int i = 0; i < clusters.Count; i++)
            {
                Cluster c = clusters[i];
                bool eligible = SoloMuertePorBarras
                    ? (c.State != HFTClusterState.Expired && c.State != HFTClusterState.Invalidated)
                    : (c.State == HFTClusterState.Active || c.State == HFTClusterState.TouchedPOC);
        ...
        private void DibujarCluster(Cluster c)
        {
            ...
            if (SoloMuertePorBarras)
            {
                endAgo = currentBar - (c.EndBar + BarrasExtensionMuerte);
            }
            else if (ProyectarAlFuturo)
            {
                endAgo = -ExtensionDibujo;
            }
            else
            {
                endAgo = currentBar - c.EndBar;
            }
...
[NinjaScriptProperty]
        ...
        // I. Consumo y Desgaste Dinámico
        [NinjaScriptProperty]
        [Display(Name="Solo muerte por barras (Extend)", Order=0, GroupName="I. Consumo y Desgaste", Description="Si es true, el cluster NO muere por consumo de volumen; vive y se extiende exactamente la cantidad de barras indicada en 'Barras extension cluster'.")]
        public bool SoloMuertePorBarras { get; set; }

        [NinjaScriptProperty][Range(10, 10000)]
        [Display(Name="Barras extension cluster", Order=1, GroupName="I. Consumo y Desgaste", Description="Cantidad fija de barras que se extiende y sobrevive el cluster antes de morir. Default 500.")]
        public int BarrasExtensionMuerte { get; set; }
