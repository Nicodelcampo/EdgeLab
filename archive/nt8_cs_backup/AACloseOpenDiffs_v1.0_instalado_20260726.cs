...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private AACloseOpenDiffs[] cacheAACloseOpenDiffs;
		public AACloseOpenDiffs AACloseOpenDiffs(int minDiffTicks, string eventLogPath, int extendBars, int baseOpacity, int maxRectangles, bool filtrarPorPercentil, double percentilMin, int ventanaPercentil, bool detectarExpansiones, int expansionMinTicks, int expansionMinBarsM1, int expansionReversalTicks, int expansionOpacity)
		{
			return AACloseOpenDiffs(Input, minDiffTicks, eventLogPath, extendBars, baseOpacity, maxRectangles, filtrarPorPercentil, percentilMin, ventanaPercentil, detectarExpansiones, expansionMinTicks, expansionMinBarsM1, expansionReversalTicks, expansionOpacity);
		}

		public AACloseOpenDiffs AACloseOpenDiffs(ISeries<double> input, int minDiffTicks, string eventLogPath, int extendBars, int baseOpacity, int maxRectangles, bool filtrarPorPercentil, double percentilMin, int ventanaPercentil, bool detectarExpansiones, int expansionMinTicks, int expansionMinBarsM1, int expansionReversalTicks, int expansionOpacity)
		{
			if (cacheAACloseOpenDiffs != null)
				for (int idx = 0; idx < cacheAACloseOpenDiffs.Length; idx++)
					if (cacheAACloseOpenDiffs[idx] != null && cacheAACloseOpenDiffs[idx].MinDiffTicks == minDiffTicks && cacheAACloseOpenDiffs[idx].EventLogPath == eventLogPath && cacheAACloseOpenDiffs[idx].ExtendBars == extendBars && cacheAACloseOpenDiffs[idx].BaseOpacity == baseOpacity && cacheAACloseOpenDiffs[idx].MaxRectangles == maxRectangles && cacheAACloseOpenDiffs[idx].FiltrarPorPercentil == filtrarPorPercentil && cacheAACloseOpenDiffs[idx].PercentilMin == percentilMin && cacheAACloseOpenDiffs[idx].VentanaPercentil == ventanaPercentil && cacheAACloseOpenDiffs[idx].DetectarExpansiones == detectarExpansiones && cacheAACloseOpenDiffs[idx].ExpansionMinTicks == expansionMinTicks && cacheAACloseOpenDiffs[idx].ExpansionMinBarsM1 == expansionMinBarsM1 && cacheAACloseOpenDiffs[idx].ExpansionReversalTicks == expansionReversalTicks && cacheAACloseOpenDiffs[idx].ExpansionOpacity == expansionOpacity && cacheAACloseOpenDiffs[idx].EqualsInput(input))
						return cacheAACloseOpenDiffs[idx];
			return CacheIndicator<AACloseOpenDiffs>(new AACloseOpenDiffs(){ MinDiffTicks = minDiffTicks, EventLogPath = eventLogPath, ExtendBars = extendBars, BaseOpacity = baseOpacity, MaxRectangles = maxRectangles, FiltrarPorPercentil = filtrarPorPercentil, PercentilMin = percentilMin, VentanaPercentil = ventanaPercentil, DetectarExpansiones = detectarExpansiones, ExpansionMinTicks = expansionMinTicks, ExpansionMinBarsM1 = expansionMinBarsM1, ExpansionReversalTicks = expansionReversalTicks, ExpansionOpacity = expansionOpacity }, input, ref cacheAACloseOpenDiffs);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.AACloseOpenDiffs AACloseOpenDiffs(int minDiffTicks, string eventLogPath, int extendBars, int baseOpacity, int maxRectangles, bool filtrarPorPercentil, double percentilMin, int ventanaPercentil, bool detectarExpansiones, int expansionMinTicks, int expansionMinBarsM1, int expansionReversalTicks, int expansionOpacity)
		{
			return indicator.AACloseOpenDiffs(Input, minDiffTicks, eventLogPath, extendBars, baseOpacity, maxRectangles, filtrarPorPercentil, percentilMin, ventanaPercentil, detectarExpansiones, expansionMinTicks, expansionMinBarsM1, expansionReversalTicks, expansionOpacity);
		}

		public Indicators.AACloseOpenDiffs AACloseOpenDiffs(ISeries<double> input , int minDiffTicks, string eventLogPath, int extendBars, int baseOpacity, int maxRectangles, bool filtrarPorPercentil, double percentilMin, int ventanaPercentil, bool detectarExpansiones, int expansionMinTicks, int expansionMinBarsM1, int expansionReversalTicks, int expansionOpacity)
		{
			return indicator.AACloseOpenDiffs(input, minDiffTicks, eventLogPath, extendBars, baseOpacity, maxRectangles, filtrarPorPercentil, percentilMin, ventanaPercentil, detectarExpansiones, expansionMinTicks, expansionMinBarsM1, expansionReversalTicks, expansionOpacity);
		}
	}
