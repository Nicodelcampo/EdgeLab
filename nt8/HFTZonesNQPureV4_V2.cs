...
namespace NinjaTrader.NinjaScript.Indicators
{
    public class HFTZonesNQPureV4_V2 : Indicator
    {
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Description              = "Detector HFT NQ/MNQ V2: Paridad estricta, monotonic zone_seq, nanosegundos, sin INSERT OR IGNORE, session_id determinista.";
        ...
        private void Finalizar()
        {
            ...
            if (validSteps >= minPasosReq && (isSweep || isAbsorb))
            {
                ...
                if (veloOk && durOk && volOk && volTotOk && limpioOk)
                {
                    ...
                    zones.Add(z);
                    if (EnableDbLogging) PersistZoneV2(z, 1, null);
                }
            }
            ResetState();
        }
            ...
            try
            {
                ...
                using (var cmd = new SQLiteCommand(
                    @"CREATE TABLE IF NOT EXISTS hft_zones_v2 (
                        ...
                        source_indicator_sha256 TEXT NOT NULL,
                ...
                // NOTA CRITICA: Se ELIMINA INSERT OR IGNORE. Colisiones lanzan excepcion visible.
                zoneCmd = new SQLiteCommand(
                    @"INSERT INTO hft_zones_v2
                      (instrument,contract,session_id,zone_seq,start_ts_ns,end_ts_ns,available_ts_ns,dir,accepted,first_failed_gate,
                       bucket,price_upper,price_lower,price_mid,height_ticks,pasos,valid_steps,avg_ms,total_ms,vol_rate,total_vol,
                       max_tick_vol,cvd_sweep,buy_vol,sell_vol,delta_slope,delta_first,delta_second,no_move_ticks,no_move_vol,
                       max_level_ticks,tick_res,max_retro,source_indicator_sha256,parameter_manifest_sha256)
                      VALUES (@inst,@ct,@sess,@zseq,@s_ns,@e_ns,@avail_ns,@dir,@acc,@ffg,@b,@pu,@pl,@pm,@ht,@p,@vs,@am,@tm,@vr,@tv,
                              @mtv,@cvd,@buy,@sell,@dsl,@d1,@d2,@nmt,@nmv,@mlt,@tr,@mr,@src_sha,@param_sha)", dbConn);
                foreach (string p in new[]{"@inst","@ct","@sess","@zseq","@s_ns","@e_ns","@avail_ns","@dir","@acc","@ffg",
                    "@b","@pu","@pl","@pm","@ht","@p","@vs","@am","@tm","@vr","@tv",
                    "@mtv","@cvd","@buy","@sell","@dsl","@d1","@d2","@nmt","@nmv","@mlt","@tr","@mr","@src_sha","@param_sha"})
        ...
        private void CheckSessionBoundary(DateTime tickTime)
        {
            ...
            if (sessionDate != lastSessionDate)
            {
                ...
                currentSessionId = sessionDate.ToString("yyyyMMdd");
                currentZoneSeq = 0; // monotonic reset per session
            }
        }

        private void PersistZoneV2(Zone z, int accepted, string firstFailedGate)
        {
            if (!dbReady) return;
            CheckSessionBoundary(z.EndTime);
            currentZoneSeq++;

            long startNs = UnixNs(z.StartTime);
            long endNs = UnixNs(z.EndTime);
            long availNs = UnixNs(Times[0][0]); // Available at bar close

            string contract = Instrument.MasterInstrument.Name + " " + Instrument.Expiry.ToString("MM-yy");

            zoneBuf.Add(new object[] {
                Instrument.FullName, contract, currentSessionId, currentZoneSeq,
                startNs, endNs, availNs, z.Direction, accepted, firstFailedGate ?? (object)DBNull.Value,
                z.Bucket.ToString(), z.Upper, z.Lower, (z.Upper + z.Lower) / 2.0, z.HeightTicks,
                z.Pasos, z.ValidSteps, z.AvgMs, z.TotalMs, z.VolRate, z.TotalVol,
                z.MaxTickVol, z.Cvd, z.BuyVol, z.SellVol, z.DeltaSlope, z.DeltaFirst, z.DeltaSecond,
                z.NoMoveTicks, z.NoMoveVol, z.MaxLevelTicks, TickResolution, z.MaxRetro,
                SOURCE_SHA256, PARAMETER_MANIFEST_SHA256 });
            FlushAll();
        }
