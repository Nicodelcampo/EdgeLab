// ============================================================================
// TickBarDiag.cs — v1.1 — Instrumental de diagnóstico TICKBAR-001
...
namespace NinjaTrader.NinjaScript.Indicators
{
	public class TickBarDiag : Indicator
	{
		...
		private bool digestArmed = false;
		...
		protected override void OnStateChange()
		{
			...
			else if (State == State.DataLoaded)
			{
				try
				{
					...
					// El nombre lleva la RESOLUCION resuelta del chart y un contador. Motivo

					// (incidente 2026-07-25): con sobrescritura a secas, cambiar el chart de

					// 25 Tick a 10 Tick sin cambiar la ruta dispara DataLoaded y PISA la

					// captura anterior. Con el sufijo eso es IMPOSIBLE: cada resolucion

					// escribe su propio archivo, y si ya existe se abre el siguiente indice.

					// Nunca se appendea (append fue lo que mezclo 3 corridas el 2026-07-24) y

					// nunca se pisa: los dos modos de falla quedan cerrados.

					string bs = BarsPeriod.BarsPeriodType.ToString() + BarsPeriod.Value.ToString(inv);
					...
					resolvedPath = Path.Combine(dir, baseName + ext);

					for (int k = 2; File.Exists(resolvedPath) && k < 1000; k++)

						resolvedPath = Path.Combine(dir, baseName + "_" + k.ToString(inv) + ext);

					log = new StreamWriter(resolvedPath, false, new UTF8Encoding(false));

					Print("TickBarDiag: escribiendo " + resolvedPath);

					log.AutoFlush = true;
					log.WriteLine("# meta indicator=TickBarDiag,version=1.1,purpose=TICKBAR-001,"
						+ "digest=fnv_basis_mix1000003_ulong,price=integer_ticks_awayfromzero,"
						+ "vol_int=round(vol*100),digest_scope=solo_eventos_de_la_ventana");
		...
		private void AccumulateTick()
		{
			...
			// El digest cubre SOLO los eventos de la ventana registrada. Antes

			// acumulaba desde el primer tick del chart (incluido el warm-up), asi que

			// Python -que solo ve la ventana- no podia reproducirlo jamas y la

			// comparacion de H1 daba DIFIERE aunque los streams fueran identicos.

			if (!digestArmed && InWindow()) { digestStream = FNV_BASIS; digestArmed = true; }
