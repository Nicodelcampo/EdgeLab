// # meta indicator=VWAP_GC,version=1.0.0
//
// VWAP_GC — Institutional Anchored VWAP con Bandas de Desviación Estándar para COMEX Gold (GC / MGC).
//
// Diseñado con Rigurosidad Cuantitativa EdgeLab:
// - Puntos de Anclaje Óptimos para Oro:
//   * COMEX Pit Open (07:20 CT / 09:20 Argentina): El anclaje reina del oro físico y financiero de EE.UU.
//   * Session CME Globex (17:00 CT / 19:00 Argentina): Sesión completa de 23h.
//   * London Bullion Fix (02:00 CT / 04:00 Argentina): Apertura del mercado físico de Londres (LBMA).
//   * Chart Session: Reinicio automático con la plantilla de sesión del gráfico.
// - Bandas de Desviación Estándar (Gaussian Value Area):
//   * Banda 1 (±1.0σ): Value Area institucional (68.2% del volumen negociado).
//   * Banda 2 (±2.0σ): Extremos de agotamiento y absorción (95.4% del volumen).
//   * Banda 3 (±3.0σ): Eventos de capitulación y fat-tail (99.7% del volumen).
...
{
	public enum VWAPAnchorGC
	{
		COMEX_Pit_0720_CT,     // 07:20 CT / 09:20 Argentina — Apertura del Pit de Oro de COMEX (Recomendado)
		Session_CME_1700_CT,   // 17:00 CT / 19:00 Argentina — Apertura de sesión electrónica global (23h)
		London_Fix_0200_CT,    // 02:00 CT / 04:00 Argentina — Apertura del mercado físico de Londres (LBMA)
	...
	public enum VWAPTimezoneGC
	{
		CME_Central_Time,      // Horarios oficiales de Chicago (CME/COMEX)
...
{
	public class VWAP_GC : Indicator
	{
		...
		protected override void OnStateChange()
		{
			...
			{
				Description					= "VWAP_GC — Institutional Anchored VWAP con Bandas de Desviación Estándar para Oro (GC / MGC)";
				Name						= "VWAP_GC";
				...
				// Anclaje recomendado para COMEX Gold: 07:20 CT (Pit Open de Nueva York)
				AnchorMode					= VWAPAnchorGC.COMEX_Pit_0720_CT;
				TimezoneMode				= VWAPTimezoneGC.CME_Central_Time;
				CustomAnchorHour			= 7;
				...
				AddPlot(new Stroke(Brushes.Crimson, DashStyleHelper.Dot, 1), PlotStyle.Line, "Upper Band 3 (+3σ)");
				AddPlot(new Stroke(Brushes.Crimson, DashStyleHelper.Dot, 1), PlotStyle.Line, "Lower Band 3 (-3σ)");
			}
		...
		protected override void OnBarUpdate()
		{
			...
			if (TimezoneMode == VWAPTimezoneGC.CME_Central_Time && _cmeTz != null)
			{
			...
			if (AnchorMode == VWAPAnchorGC.Chart_Session)
			{
			...
			else
			{
				...
				switch (AnchorMode)
				{
					case VWAPAnchorGC.COMEX_Pit_0720_CT:
						targetAnchor = new TimeSpan(7, 20, 0);
						break;
					case VWAPAnchorGC.Session_CME_1700_CT:
						targetAnchor = new TimeSpan(17, 0, 0);
						break;
					case VWAPAnchorGC.London_Fix_0200_CT:
						targetAnchor = new TimeSpan(2, 0, 0);
						break;
					case VWAPAnchorGC.Custom_Time:
					default:
			...
			if (ShowHud && CurrentBar >= 1)
			{
				...
				if (Math.Abs(numSigmas) <= Band1Multiplier)
					...
					zoneState = "SOBRECOMPRA (+2σ Agotamiento / Reversión)";
				else if (numSigmas < -Band2Multiplier)
					zoneState = "SOBREVENTA (-2σ Absorción / Reversión)";
				else if (numSigmas > 0)
					zoneState = "EXTENSIÓN ALCISTA (+1σ)";
				else
					zoneState = "EXTENSIÓN BAJISTA (-1σ)";

				string anchorDesc = AnchorMode == VWAPAnchorGC.COMEX_Pit_0720_CT ? "COMEX Pit (07:20 CT / 09:20 ARG)" :
									(AnchorMode == VWAPAnchorGC.Session_CME_1700_CT ? "CME Globex (17:00 CT / 19:00 ARG)" :
									(AnchorMode == VWAPAnchorGC.London_Fix_0200_CT ? "London Fix (02:00 CT / 04:00 ARG)" :
									(AnchorMode == VWAPAnchorGC.Chart_Session ? "Sesión Chart NT8" : string.Format("Custom {0:D2}:{1:D2}", CustomAnchorHour, CustomAnchorMinute))));

				string hudText = string.Format(
					"=== VWAP INSTITUCIONAL (GC) ===\n" +
					"Anclaje      : {0}\n" +
					"VWAP         : {1:F2}\n" +
				...
				Draw.TextFixed(this, "VWAP_GC_HUD", hudText, HudPosition,
					Brushes.White, new NinjaTrader.Gui.Tools.SimpleFont("Consolas", 10),
					Brushes.Gold, Brushes.Black, 80);
			}
		}
	}
}
