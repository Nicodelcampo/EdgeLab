# Informe Integral de Auditoría y Certificación: Régimen Contractual Multi-Activo (V2)
...
- **Estado Técnico:** `PARTIAL_MULTI_ASSET_CERTIFICATION`
  - **Certificados por Calendario Oficial CME (5/11 activos):** `ES`, `MES`, `NQ`, `YM` plenamente certificados. `MNQ` certificado condicionado a exclusión de gaps.
  - **En Abstención por Falta de Calendario Oficial (6/11 activos):** `6B`, `6E`, `6J`, `GC`, `ZB`, `MBT` (`ABSTAIN_CALENDAR_EVIDENCE_REQUIRED`).

---

## 1. Resumen Ejecutivo y Alcance

Se auditó el universo completo de datos de futuros pre-holdout disponibles en EdgeLab, abarcando **11 raíces de productos**, **56 archivos parquet** y **1.015.108.799 filas** con un volumen agregado de **1.407.249.709 contratos**.

Se implementó y certificó la capa causal de contratos operables bajo el estándar `previous_complete_session_volume_leader_monotonic_v1` (`CONTRACT_REGIME_V2`), garantizando:
...
4. **Frontera de Estado Post-Ordenamiento**: Asignación mandatoria de `state_reset_flag = True` calculada **estrictamente después** del ordenamiento global por `(ts_utc_ns, sequence)`.
5. **Completitud Fail-Closed y Exclusión de Mantenimiento**:
   - `completeness` defaults a `UNKNOWN` (requiere evidencia positiva para aprobar).
   - Ticks de la ventana de mantenimiento CME (16:00 a 17:00 CT) son filtrados determinísticamente y no computan para volumen de sesión regular.
...
A partir del escaneo seguro consolidado en [`UNIVERSAL_CONTRACT_INVENTORY_2026-09-15.json`](UNIVERSAL_CONTRACT_INVENTORY_2026-09-15.json), la distribución por raíz es la siguiente:
...
| **MNQ** | Equity Index Micro | 5 | 334.506.728 | 381.459.605 | 2025-08-01 → 2026-06-30 | **CERTIFIED (Gaps)** | `375b20504045e5ee...` |
...
3. Se auditó la fuente de verdad primaria en `data/nt8_research_v2/GC_parquet`:
...
1. **Compuerta `ContractSessionEligibilityGateV2`**: Incorpora los métodos deterministas `aggregate_session_ticks` y `evaluate_session_from_ticks` que identifican cualquier transacción ocurrida entre las 16:00:00 y las 16:59:59.999 CT.
...
1. **MNQ (Micro E-mini Nasdaq)**:
   ...
   - Conforme al principio de fail-closed, el motor de régimen evalúa esos periodos como `SOURCE_INCOMPLETE` y se abstiene de declarar un contrato activo en esas fechas. Para usar `MNQ` en investigaciones downstream (como HP-007), se exige la exclusión explícita de estos dos gaps.
...
## 4. Arquitectura de Módulos y Reconciliación de Tests

Se implementaron y probaron exhaustivamente los siguientes componentes:

1. [`edgelab/data/contract_session_gate.py`](../../edgelab/data/contract_session_gate.py):
   - Compuerta causal multi-activo `ContractSessionEligibilityGateV2`.
   - Resolución portable de calendarios relativos a la raíz del repositorio.
   - `completeness` fail-closed por defecto (`UNKNOWN`).
   - Filtrado y descuento explícito de la ventana de mantenimiento (16:00 a 17:00 CT).
2. [`edgelab/data/continuous_contract.py`](../../edgelab/data/continuous_contract.py):
   - Generador causal continuo `build_continuous_series`.
   - Ensambla `<ROOT>_CONT_CAUSAL_D1.parquet` con precios reales y **10 columnas obligatorias de linaje** (`root, contract, trade_date, regime_id, roll_manifest_sha256, source_file, source_row, ts_utc_ns, sequence, state_reset_flag`).
   - `state_reset_flag` asignado **después del ordenamiento global** por `(ts_utc_ns, sequence)`.
   - Exige `sequence` obligatoria y valida unicidad de pares `(ts_utc_ns, sequence)`.
   - Validación de contenido interno (`instrument`, `contract`) prohibiendo mezclas micro/estándar.
...
1. [`SESSION_COMPLETENESS_INDEX_2026-09-15.md`](SESSION_COMPLETENESS_INDEX_2026-09-15.md): Índice compacto de auditoría y consulta rápida (diseñado para preservar el contexto de agentes).
2. [`UNIVERSAL_CONTRACT_INVENTORY_2026-09-15.json`](UNIVERSAL_CONTRACT_INVENTORY_2026-09-15.json): Inventario criptográfico y métrico de los 56 archivos parquet.
3. [`UNIVERSAL_CONTRACT_REGIME_V2_SPEC_2026-09-15.md`](UNIVERSAL_CONTRACT_REGIME_V2_SPEC_2026-09-15.md): Especificación técnica formal del estándar V2.
4. [`UNIVERSAL_SESSION_COMPLETENESS_2026-09-15.json`](UNIVERSAL_SESSION_COMPLETENESS_2026-09-15.json): Matriz consolidada de 3.833 evaluaciones de completitud de sesión.
5. Manifiestos de régimen validados por activo en [`contract_regimes/`](contract_regimes/):
