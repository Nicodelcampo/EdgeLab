...
**Estado:** prototipo de planificador + comparador de evaluación; pendiente de CI/revisión. No es un daemon ni modifica NORTH_STAR/G0–G5.  
**Objetivo:** que un ciclo deje capacidad reutilizable y verificable para los siguientes, sin medir progreso por CPU, tokens o número de tareas.

## Hallazgo de research aplicado
...
Se agregó `edgelab/edge_brain/recursive_eval.py`:
...
- Los tests cargan los dos módulos por ruta directa para aislarlos del inicializador amplio de `edgelab.edge_brain`.

El chequeo de SHA es integridad, **no firma ni certificación de que un runner confiable ejecutó el ensayo**. El comparador tampoco puede impedir que el harness exponga development scores, que alguien itere repetidamente sobre ellos, o que re-use la auditoría final. Hace falta un runner aislado, custodia de split/ACL y un ledger con consumo one-shot del audit para cerrar ese hueco. Hasta entonces queda como prototipo para inspección; ningún output del comparador es prueba de capacidad o de edge.
...
No afirmar "exponencial" por una sola racha. Medir log-yield por unidad de recurso en una serie de ciclos, fijar comparadores antes de observarlos, ensayar contra incumbente con el mismo presupuesto, separar el grader/holdout del agente que propone, y reportar incertidumbre y coste de falsos positivos. El sistema puede estancarse o caer; stop-and-diagnose es parte del diseño.
...
5. CI y grader verifican resultado sin dar acceso al sealed audit al proposer.
6. Solo los candidatos de desarrollo pueden nominarse; audit separado de una vez decide si se reemplaza el incumbent.
7. Brain registra proposal, evaluación, outcome, costo, código/datos y lección; aprobación separada para memoria durable.
...
**Aporte al referente:** la segunda iteración agrega comparación emparejada y de presupuesto fijo, checksums de protocolo/recibo, separación selection/development, y la regla clave de no promover desde desarrollo; define los límites concretos que faltan antes de un ciclo unattended.
