...
16. **Prop firms — EV del participante (2026-09-26, rama `feat/propfirm-ev-20260926`).** `docs/research/PROPFIRM_EV_DISENO_20260926.md`. Scraper de reglas (candidatos con sha256, verificación humana), reglas como datos, Monte Carlo del ciclo completo con control de ventaja cero (Hall 2026) y requisito de ventaja neta por trade (`edgelab/propfirm/`). Cruce: las ventajas de ticks chicos medidas en EdgeLab no pasan ninguna evaluación; las de objetivos grandes darían EV > 0 pero no están validadas, y `gate()` (EV en el IC inferior) las rechaza. **Reglas detalladas** (`docs/research/PROPFIRM_REGLAS_DETALLADAS_20260926.md`): crawler + taxonomía de 36 categorías + Zendesk API + Firecrawl; reglas de retiro escondidas simuladas (topes por número, colchón, días calificados, retiro mínimo, cierre tras N retiros, acceso 30 días). **Apex y TakeProfitTrader prohíben algoritmos: no aplican a EdgeLab.**
