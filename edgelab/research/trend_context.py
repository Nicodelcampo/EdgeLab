...
def _digest(p): return hashlib.sha256(json.dumps(p,sort_keys=True,separators=(",",":")).encode()).hexdigest()
def _mean(q,n): return None if len(q)<n else sum(q)/n
def compute_trend_contexts(bars:Iterable[TrendBar],*,params=TrendParameters(),availability_lag_ns=1):
 ...
 for idx,b in enumerate(rows):
  ...
  c=float(b.close_ticks); ef=c if ef is None else af*c+(1-af)*ef; es=c if es is None else as_*c+(1-as_)*es; fw.append(c); sw.append(c); sf=_mean(fw,params.sma_fast); ss=_mean(sw,params.sma_slow)
  if b.volume>0:
   tp=c if b.typical_price_ticks is None else float(b.typical_price_ticks); cpv+=tp*float(b.volume); cv+=float(b.volume)
  vw=cpv/cv if cv>0 else None; eh.append(ef); sv.append(vw); e_sl=ef-eh[-1-params.slope_lookback] if len(eh)>params.slope_lookback else None; v_sl=None
  if len(sv)>params.slope_lookback and vw is not None and sv[-1-params.slope_lookback] is not None: v_sl=vw-sv[-1-params.slope_lookback]
  ready=all(x is not None for x in (sf,ss,vw,e_sl,v_sl))
  if not ready: e_state=s_state=v_state=state=TrendState.NOT_READY
  else:
   e_state=TrendState.UP if ef>es and e_sl>0 else TrendState.DOWN if ef<es and e_sl<0 else TrendState.NEUTRAL
   s_state=TrendState.UP if sf>ss else TrendState.DOWN if sf<ss else TrendState.NEUTRAL
   v_state=TrendState.UP if c>vw and v_sl>0 else TrendState.DOWN if c<vw and v_sl<0 else TrendState.NEUTRAL
   state=TrendState.UP if e_state is s_state is v_state is TrendState.UP else TrendState.DOWN if e_state is s_state is v_state is TrendState.DOWN else TrendState.NEUTRAL
  p=dict(contract_version=CONTRACT_VERSION,ts_ns=b.ts_ns,available_at_ns=b.ts_ns+availability_lag_ns,session_id=b.session_id,bar_index=idx,session_bar_index=sidx,close_ticks=c,vwap_ticks=vw,ema_fast_ticks=ef,ema_slow_ticks=es,sma_fast_ticks=sf,sma_slow_ticks=ss,ema_slope_ticks=e_sl,vwap_slope_ticks=v_sl,close_minus_vwap_ticks=c-vw if vw is not None else None,close_minus_ema_fast_ticks=c-ef,close_minus_ema_slow_ticks=c-es,ema_state=e_state.value,sma_state=s_state.value,vwap_state=v_state.value,trend_state=state.value)
  out.append(TrendContext(CONTRACT_VERSION,b.ts_ns,b.ts_ns+availability_lag_ns,b.session_id,idx,sidx,c,vw,ef,es,sf,ss,e_sl,v_sl,p["close_minus_vwap_ticks"],p["close_minus_ema_fast_ticks"],p["close_minus_ema_slow_ticks"],e_state,s_state,v_state,state,_digest(p)))
 return out
def align_event_with_trend(continuation_direction,context,*,event_available_at_ns):
 if continuation_direction not in (-1,1): raise ValueError("continuation_direction must be +1 or -1")
 ...
 return EventAlignment.WITH_TREND if continuation_direction==(1 if context.trend_state is TrendState.UP else -1) else EventAlignment.COUNTER_TREND
