...
import numpy as np
NS=1_000_000_000; WEBB=np.array([-np.sqrt(1.5),-1,-np.sqrt(.5),np.sqrt(.5),1,np.sqrt(1.5)])
@dataclass(frozen=True)
class FirstPassageResult:
 outcome:str; score:int; fill_idx:int; end_idx:int; cap_driver:str; target_level_ticks:int; stop_level_ticks:int; first_touch_idx:int|None; first_touch_ts_utc_ns:int|None; first_touch_source_row:int|None; touch_price_ticks:int|None; ticks_to_touch:int|None; seconds_to_touch:float|None
 def to_dict(self): return asdict(self)
def _arrays(price,ts,source,sessions):
 p=np.asarray(price,dtype=np.int64); t=np.asarray(ts,dtype=np.int64); r=np.asarray(source,dtype=np.int64); s=np.asarray(sessions)
...
def session_last_indices(sessions):
 ...
 cuts=np.flatnonzero(s[1:]!=s[:-1])+1; starts=np.r_[0,cuts]; stops=np.r_[cuts,n]; out=np.empty(n,dtype=np.int64)
...
def horizon_endpoints(ts,sessions,*,tick_cap,clock_cap_seconds):
 ...
 idx=np.arange(n,dtype=np.int64); last=session_last_indices(s); choices=[last]; names=["session"]; priority=[2]
 ...
 if clock_cap_seconds is not None:
  ...
  deadline=t+int(round(float(clock_cap_seconds)*NS)); choices.append(np.searchsorted(t,deadline,side="right")-1); names.append("clock"); priority.append(1)
...
class _Tree:
 ...
 def update(self,pos,value):
  ...
  while node: self.data[node]=min(self.data[2*node],self.data[2*node+1]); node//=2
...
def next_barrier_touch_indices(price,sessions,*,barrier_ticks):
 ...
 up=np.full(n,-1,dtype=np.int64); down=np.full(n,-1,dtype=np.int64); cuts=np.flatnonzero(s[1:]!=s[:-1])+1 if n>1 else np.array([],dtype=np.int64)
 for lo,hi in zip(np.r_[0,cuts],np.r_[cuts,n]):
  ...
  for idx in range(int(hi)-1,int(lo)-1,-1):
   pos=int(p[idx])-low; u=pos+b; d=pos-b; uh=tree.query(u,high-low) if u<=high-low else inf; dh=tree.query(0,d) if d>=0 else inf
   if uh!=inf: up[idx]=uh
...
def first_passage_scores_fast(price,ts,sessions,*,fill_indices,directions,barrier_ticks,tick_cap,clock_cap_seconds,precomputed_touches=None,precomputed_endpoints=None):
 ...
 up,down=precomputed_touches if precomputed_touches is not None else next_barrier_touch_indices(p,sessions,barrier_ticks=barrier_ticks); ends=precomputed_endpoints
 if ends is None: ends,_=horizon_endpoints(ts,sessions,tick_cap=tick_cap,clock_cap_seconds=clock_cap_seconds)
 end=np.asarray(ends)[idx]; target=np.where(direction>0,up[idx],down[idx]); stop=np.where(direction>0,down[idx],up[idx]); target_ok=(target>=0)&(target<=end); stop_ok=(stop>=0)&(stop<=end)
 if np.any(target_ok&stop_ok&(target==stop)): raise AssertionError("simultaneous scalar barrier touch")
 out=np.zeros(len(idx),dtype=np.int8); out[target_ok&(~stop_ok|(target<stop))]=1; out[stop_ok&(~target_ok|(stop<target))]=-1; return out
def first_passage(price,ts,source,sessions,*,fill_idx,direction,target_ticks,stop_ticks,tick_cap=None,clock_cap_seconds=None):
 ...
 end,driver=horizon_endpoint(t,s,fill_idx=i,tick_cap=tick_cap,clock_cap_seconds=clock_cap_seconds); target=int(p[i])+d*int(target_ticks); stop=int(p[i])-d*int(stop_ticks); touch=None; outcome="TIMEOUT"
...
def summarize_scores(scores:Iterable[int]):
 ...
 tp=int(np.sum(x==1)); sl=int(np.sum(x==-1)); timeout=int(np.sum(x==0)); resolved=tp+sl; n=len(x); return {"n":n,"n_tp_first":tp,"n_sl_first":sl,"n_timeout":timeout,"p_tp_first":tp/n,"p_sl_first":sl/n,"p_timeout":timeout/n,"p_tp_given_resolved":tp/resolved if resolved else None,"theta_fp":float(x.mean())}
def wild_cluster_test(values:Iterable[float],*,replications,seed,confidence=.95):
 ...
 alpha=(1-confidence)/2; p=(1+np.sum(np.abs(null)>=abs(point)))/(replications+1); return {"point":point,"lower":float(np.quantile(ci,alpha)),"upper":float(np.quantile(ci,1-alpha)),"p_two_sided":float(p),"n_sessions":len(x),"replications":replications}
