"""Checkpointed, memory-bounded five-contract Gate 1 replication.

Each selected CME session runs in a fresh process with the immediately preceding
valid session as causal warm-up. Checkpoints make the 234-session run resumable
and ensure a large contract is never loaded as one in-memory object.
"""
...
def _raw_abs_events(ticks, sessions, allowed):
 from edgelab.research.all5_runtime.bigtrap2absorption import DEFAULTS, run
...
def _context(root:Path):
 sr=_expand_registry(_load(root/'specs/bt2_absorption_gate1_all5_sessions_2026-08-26.json'));ir=_load(root/'specs/bt2_absorption_gate1_all5_input_registry_2026-08-26.json');sp=_load(root/'specs/bt2_absorption_gate1_all5_post_outcome_replication_2026-08-26.json');return sr,ir,sp
...
def compute_session(*,repo_root:Path,data_dir:Path,index:int)->dict[str,Any]:
 ...
 sindex={(c,r['cme_session_id']):i for c in contracts for i,r in enumerate(sorted(by[c],key=lambda x:x['cme_session_id']))};cindex={c:i for i,c in enumerate(contracts)};seed=int(sp['randomization']['seed']);reps=int(sp['randomization']['replications']);ticks=load_canonical_parquet(data_dir/ir['contracts'][contract]['parquet_file'],contract=contract,start_utc_ns=_start(warmup),end_utc_ns=_end(session),instrument='GC');labels=_labels(ticks.ts_ns);cache=build_path_cache(ticks.ts_ns,ticks.price_ticks,labels,tick_cap=int(sp['horizon']['tick_cap']),clock_cap_seconds=int(sp['horizon']['clock_cap_seconds']));rawa=_raw_abs_events(ticks,labels,{session});rawb=_raw_bt2_events(ticks,labels,{session});ae,exa=attach_fills(rawa,ts_ns=ticks.ts_ns,source_row=ticks.sequence,session_ids=labels);be,exb=attach_fills(rawb,ts_ns=ticks.ts_ns,source_row=ticks.sequence,session_ids=labels);ae=[e for e in ae if cache.eligible[e.fill_idx]];be=[e for e in be if cache.eligible[e.fill_idx]];aes=_event_session_estimates(ae,ticks.price_ticks,cache);bes=_event_session_estimates(be,ticks.price_ticks,cache)
...
def _build_result(*,rows,counts,exclusions,sr,sp,provenance):
...
def finalize(*,repo_root:Path,checkpoint_dir:Path,output_dir:Path)->dict[str,Any]:
 ...
 prov={'runner':'edgelab.research.bt2_gate1_all5','execution':'fresh_process_per_session_checkpointed','sample_registry_sha256':sr['registry_payload_sha256'],'input_registry_sha256':ir['registry_payload_sha256'],'spec_file':'specs/bt2_absorption_gate1_all5_post_outcome_replication_2026-08-26.json'};result=_build_result(rows=rows,counts=counts,exclusions=exclusions,sr=sr,sp=sp,provenance=prov);raw=json.dumps(result,sort_keys=True,separators=(',',':')).encode();result['result_payload_sha256']=hashlib.sha256(raw).hexdigest();output_dir.mkdir(parents=True,exist_ok=True);tmp=output_dir/'gate1_all5_result.json.tmp';final=output_dir/'gate1_all5_result.json';tmp.write_text(json.dumps(result,indent=2,ensure_ascii=False)+'\n');tmp.replace(final);print(json.dumps({'status':result['status'],'decision':result['decision'],'n_sessions':result['n_sessions'],'contrasts':result['contrasts'],'result_payload_sha256':result['result_payload_sha256']},indent=2),flush=True);return result
def main()->int:
