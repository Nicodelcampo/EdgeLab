...
def test_zero_duration_tick_bucket_is_valid_when_ordering_is_causal():assert run(event(formation_start_ns=200,formation_end_ns=200,available_at_ns=200),[Tick(201,10,210,"S1")],Policy("P0","IMMEDIATE")).state=="ENTERED"
...
def test_formation_touch_is_excluded():
 d=run(event(),[Tick(150,1,202,"S1"),Tick(201,2,208,"S1"),Tick(202,3,204,"S1"),Tick(203,4,206,"S1")],Policy("R","RETEST",departure_ticks=2,depth=0,max_wait_ticks=10));assert(d.trigger_ts_ns,d.entry_ts_ns)==(202,203)
...
def test_retest_without_later_tick_abstains():
 d=run(event(),ticks(208,204),Policy("R","RETEST",departure_ticks=2,depth=0,max_wait_ticks=10));assert d.state=="CENSORED"and d.censor_reason=="NO_EXECUTABLE_FILL_AVAILABLE"and d.trigger_ts_ns==202 and d.entry_ts_ns is None
