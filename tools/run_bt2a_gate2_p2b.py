...
AUTH="AUTHORIZE_BT2A_P2B_POST_OUTCOME_DIAGNOSTIC"; BARRIERS=(5,9,18,30); TIME_STOPS=(5,30,120); ARMS=("K_ABS","K_BT2")
...
def run_session(root,data_dir,event_store,out,index,commission,tick_value,commission_source):
 registry,inputs,_=_context(root); row=registry["sessions"][int(index)]; contract=row["contract"]; session=row["cme_session_id"]; source=json.loads((Path(event_store)/"checkpoints"/f"session_{int(index):03d}.json").read_text()); events=validate_event_checkpoint(source,contract=contract,session=session,sample_registry_sha256=registry["registry_payload_sha256"],input_registry_sha256=inputs["registry_payload_sha256"]); parquet=Path(data_dir)/inputs["contracts"][contract]["parquet_file"]; verify_file_sha256(parquet,inputs["contracts"][contract]["parquet_sha256"]); ticks=load_canonical_parquet(parquet,contract=contract,start_utc_ns=_start(session),end_utc_ns=_end(session),instrument="GC"); labels=_labels(ticks.ts_ns)
 ...
 for arm in ARMS:
  arm_events=[e for e in events if e["arm"]==arm]
  for barrier in BARRIERS:
   for seconds in TIME_STOPS:
    signals=[{"event_id":e["event_id"],"signal_ts_utc_ns":e["signal_ts_utc_ns"],"signal_source_row":e["signal_source_row"],"direction":e["direction"],"target_ticks":barrier,"stop_ticks":barrier,"time_stop_seconds":seconds} for e in arm_events]
    for name,cost in costs.items():
     result=simulate(signals,ticks.ts_ns,ticks.sequence,ticks.price_ticks,ticks.bid_ticks,ticks.ask_ticks,labels,cost=cost,tick_value_usd=tick_value,close_at_session_end=True); cells.append({"arm":arm,"barrier_ticks":barrier,"time_stop_seconds":seconds,"scenario":name,"summary":result["summary"],"trades":result["trades"],"rejected":result["rejected"],"digest":result["digest"]})
  ...
  for barrier in BARRIERS:
   for seconds in TIME_STOPS:
    for scenario in ("ideal","base","adverso","severo"):
     cells=[next(c for c in row["cells"] if c["arm"]==arm and c["barrier_ticks"]==barrier and c["time_stop_seconds"]==seconds and c["scenario"]==scenario) for row in rows]; inference={}
     for metric in ("mean_net_ticks_per_eligible_signal","mean_net_ticks_per_trade","mean_net_usd_per_eligible_signal","mean_net_usd_per_trade"):
      values=[c["summary"][metric] for c in cells if c["summary"][metric] is not None]; inference[metric]=wild_cluster_test(values,replications=bootstrap,seed=seed(f"{metric}|{arm}|{barrier}|{seconds}|{scenario}")) if len(values)>=2 else {"status":"INSUFFICIENT_SESSIONS","n_sessions":len(values)}
     surfaces.append({"arm":arm,"barrier_ticks":barrier,"time_stop_seconds":seconds,"scenario":scenario,"n_sessions":len(cells),"inference":inference,"total_trades":sum(c["summary"]["n_trades"] for c in cells),"total_rejected":sum(c["summary"]["n_rejected"] for c in cells)})
 commission,commission_source,tick_value=list(sources)[0]; result={"schema":"bt2a_gate2_p2b_result_v1","status":"COMPLETE_P2B_POST_OUTCOME_DIAGNOSTIC","n_sessions":len(rows),"cost_identity":{"commission_per_side_usd":commission,"commission_source":commission_source,"tick_value_usd":tick_value},"surfaces":surfaces,"unit":"CME_SESSION","one_position":True,"first_executable_signal_wins":True,"inference_scope":"DESCRIPTIVE_POST_OUTCOME_SENSITIVITY_NO_CELL_SELECTION","CAMPAIGN_OUTCOMES_OPENED":True,"EDGE_DECLARED":False,"confirmatory_eligible":False,"promotion_eligible":False}; result["payload_sha256"]=canonical(result); atomic(Path(out)/"gate2_p2b_result.json",result); return result
def preflight(root,event_store,commission,source,tick_value):
 registry,_,_=_context(root); missing=[i for i in range(len(registry["sessions"])) if not (Path(event_store)/"checkpoints"/f"session_{i:03d}.json").is_file()]; spec=Path(root)/"specs"/"bt2a_gate2_first_passage_v1.json"; status=json.loads(spec.read_text()).get("status") if spec.is_file() else "MISSING"; return {"status":"PASS_READY_FOR_AUTHORIZATION" if not missing and str(status).startswith("FROZEN") and commission is not None and bool(source) and tick_value is not None else "NOT_READY","spec_status":status,"missing_event_checkpoints":len(missing),"commission_supplied":commission is not None,"commission_source_supplied":bool(source),"tick_value_supplied":tick_value is not None,"outcomes_opened":False}
def main():
