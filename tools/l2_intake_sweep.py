...
Idempotente: una sesión con manifest ya existente se saltea. No borra CSV salvo
...
def sweep(*, csv_dir: Path, base: Path, instrument: str, contract: str, tick_size: float,
          downloader_id: str, log: Path, out: Path, delete_csv_after_validation: bool = False) -> dict:
    records = []
    ...
    continuity = (validate_boundaries(base, out, tick_size, skip_book_check=True)
                  if n_manifests >= 2 else {"pass_count": 0, "review_count": 0, "fail_count": 0,
