...
RAW=Path(os.environ.get('EDGELAB_YM_RAW_ROOT','/data/raw/ym-kaggle/extracted'))
OUT=Path(os.environ.get('EDGELAB_ECONOMIC_OUT','/data/analysis/ym-bt2a-economic/stage1')); OUT.mkdir(parents=True,exist_ok=True)
DISCOVERY_END_NS=1767225600000000000
FILES=['YM_09-25_ticks.parquet','YM_12-25_ticks.parquet','YM_03-26_ticks.parquet']
COLS=['ts_utc_ns','sequence','price_ticks','bid_ticks','ask_ticks','volume']
POLICIES=entry_policy_grid_v2(); EXITS=coarse_exit_grid_v1(); NP=len(POLICIES); NX=len(EXITS)
assert (NP,NX,NP*NX)==(132,54,7128)
count=np.zeros((NP,NX),dtype=np.int64); sums=np.zeros((NP,NX)); sums2=np.zeros((NP,NX)); wins={c:np.zeros((NP,NX),dtype=np.int64) for c in (2,4,6)}
entry_counts=np.zeros(NP,dtype=np.int64); contract_stats=defaultdict(lambda:{'events':0,'zones':0,'entries':0}); source_hashes={}
...
def gross_vector(prices,entry_idx,direction):
 sign=1 if direction=='long' else -1; end=min(len(prices),entry_idx+1001); rel=sign*(prices[entry_idx+1:end]-prices[entry_idx])
...
def entry_indices(z,prices,sessions):
 ...
 lo2=int(round(float(z['lo'])*2));hi2=int(round(float(z['hi'])*2));direction=str(z['dir']);future2=future*2
 lookup={(p.departure_ticks,p.depth,p.max_wait_ticks):i for i,p in enumerate(POLICIES) if p.mode=='RETEST'}
 for dep in (1,2,4,6,8):
  ...
  for depth in (0.0,.25,.5,.75,1.0):
   raw=hi2-depth*(hi2-lo2) if direction=='long' else lo2+depth*(hi2-lo2);target=_ceil_even(raw) if direction=='long' else _floor_even(raw)
   hit=first((tail>=lo2)&(tail<=target)) if direction=='long' else first((tail>=target)&(tail<=hi2))
...
for name in FILES:
 ...
 ts=col('ts_utc_ns',np.int64);seq=col('sequence',np.int64);prices=col('price_ticks',np.int64);bid=col('bid_ticks',np.int64);ask=col('ask_ticks',np.int64);vol=col('volume',np.float64)
 contract=pq.ParquetFile(path).read_row_group(0,columns=['contract'])['contract'][0].as_py();ticks=TickSeries(ts,prices,vol,bid,ask,seq,1.0,'YM',str(contract),str(path));sessions=session_ids(ts);run=bt2.run(ticks,params=bt2.DEFAULTS)
 ...
 for z in run['zones']:
  ...
  for pi,ei in entries.items():
   if ei not in cache:cache[ei]=gross_vector(prices,ei,str(z['dir']))
   gross=cache[ei]
...
for rank,i in enumerate(order):
 adj=min(1.0,(m-rank)*rows[i]['iid_p_one_sided_net4']);prev=max(prev,adj);rows[i]['holm_p']=prev
prev=1
for rev,i in enumerate(reversed(order),start=1):
 rank=m-rev+1;q=min(prev,rows[i]['iid_p_one_sided_net4']*m/rank);rows[i]['bh_q']=q;prev=q
...
selected=sorted(entry_best.values(),key=lambda r:(r['mean_net_6'],r['iid_lcb95_net4']),reverse=True)[:12]
summary={'schema':'ym_bt2a_economic_stage1_v1','campaign_id':'CAMP-YM-BT2A-ECONOMIC-002','split':'DISCOVERY_ONLY_BEFORE_2026','discovery_end_ns_exclusive':DISCOVERY_END_NS,'holdout_accessed':False,'validation_outcomes_accessed':False,'rows':total_rows,'zones':total_zones,'entry_policy_count':NP,'exit_spec_count':NX,'family_count':m,'minimum_trades':200,'source_hashes':source_hashes,'contracts':dict(contract_stats),'selected_entry_archetypes':selected,'top_100':top,'survivors_positive_net6':sum(1 for r in eligible if r['mean_net_6']>0),'survivors_holm_net4':sum(1 for r in eligible if r['holm_p']<.05 and r['mean_net_4']>0),'runtime_seconds':time.time()-start,'completed_at_utc':datetime.now(timezone.utc).isoformat(),'limitations':['IID screen only; session-cluster inference required before validation','Independent event outcomes; non-overlap enforcement required for final candidates','No validation or holdout outcomes opened']}
raw=json.dumps(summary,sort_keys=True,separators=(',',':')).encode();summary['evidence_sha256']=hashlib.sha256(raw).hexdigest();(OUT/'stage1_summary.json').write_text(json.dumps(summary,indent=2,sort_keys=True))
with (OUT/'stage1_all_7128.csv').open('w',newline='') as f:
