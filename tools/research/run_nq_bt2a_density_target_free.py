...
RAW=Path('/data/raw/nq-kaggle/extracted');OUT=Path('/data/analysis/nq-density-target-free');OUT.mkdir(parents=True,exist_ok=True);FILES=['NQ_09-25_ticks.parquet','NQ_12-25_ticks.parquet','NQ_03-26_ticks.parquet'];COLS=['ts_utc_ns','sequence','price_ticks','bid_ticks','ask_ticks','volume','aggressor'];MAX_TICKS_PER_SLICE=1_000_000
def file_hash(p):
 h=sha256()
 with open(p,'rb')as f:
...
start=time.time();slices=[];A={k:[]for k in('width','strength','volume','fraction','density1','density4','confluence1','confluence4')};source_hashes={};tot={'ticks':0,'zones':0,'buy':0,'sell':0,'unclassified':0,'mismatch':0}
for name in FILES:
 ...
 for month,a,b in month_slices(mn,mx):
  ...
  for i,z in enumerate(zones):
   dirs[z.direction]+=1;metrics['width'].append(z.hi_ticks-z.lo_ticks);metrics['strength'].append(z.strength_ratio);metrics['volume'].append(z.volume);metrics['fraction'].append(z.trap_fraction);center=(z.lo_ticks+z.hi_ticks)/2;hist=zones[:i+1]
   for sigma,dk,ck in((1.,'density1','confluence1'),(4.,'density4','confluence4')):
    metrics[dk].append(float(density_profile(hist,asof_idx=z.available_idx,asof_ns=z.available_ns,origin_price_ticks=center,trade_direction=z.direction,offsets=[0],sigma_ticks=sigma,scope='support')[0]));metrics[ck].append(sum(1 for old in hist if old.direction==z.direction and old.available_idx<=z.available_idx and max(old.lo_ticks-center,center-old.hi_ticks,0)<=sigma))
...
summary={'schema':'nq_bt2a_density_target_free_v1','episode_id':'EPISODE-NQ-BT2A-DENSITY-REPRESENTATION-008','unit_of_analysis':'contract_month_reset_first_1000000_ticks','sampling_contract':'FIRST_1000000_TICKS_PER_CONTRACT_MONTH','max_ticks_per_slice':MAX_TICKS_PER_SLICE,'formation_spec':'tick_count:25','discovery_end_exclusive_ns':DISCOVERY_END_NS,'outcomes_accessed':False,'validation_accessed':False,'holdout_accessed':False,'source_hashes':source_hashes,'totals':tot|{'mismatch_rate':tot['mismatch']/max(1,tot['buy']+tot['sell'])},'aggregate':{k+'_quantiles':q(v)for k,v in A.items()},'slices':slices,'runtime_seconds':time.time()-start,'completed_at_utc':datetime.now(timezone.utc).isoformat()};raw=json.dumps(summary,sort_keys=True,separators=(',',':')).encode();summary['evidence_sha256']=sha256(raw).hexdigest();(OUT/'summary.json').write_text(json.dumps(summary,indent=2,sort_keys=True));print(json.dumps({'ticks':tot['ticks'],'zones':tot['zones'],'slices':len(slices),'mismatch_rate':summary['totals']['mismatch_rate'],'evidence_sha256':summary['evidence_sha256'],'runtime_seconds':round(summary['runtime_seconds'],2)},indent=2),flush=True)
