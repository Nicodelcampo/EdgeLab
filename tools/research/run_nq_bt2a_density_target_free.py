...
RAW=Path('/data/raw/nq-kaggle/extracted');OUT=Path('/data/analysis/nq-density-target-free');OUT.mkdir(parents=True,exist_ok=True);FILES=['NQ_09-25_ticks.parquet','NQ_12-25_ticks.parquet','NQ_03-26_ticks.parquet'];COLS=['ts_utc_ns','sequence','price_ticks','bid_ticks','ask_ticks','volume','aggressor']
def file_hash(p):
 h=sha256()
 with open(p,'rb')as f:
...
for name in FILES:
 ...
 for month,a,b in month_slices(mn,mx):
  t=pq.read_table(p,columns=COLS,filters=[('ts_utc_ns','>=',a),('ts_utc_ns','<',b)])
