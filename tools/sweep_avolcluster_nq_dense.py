# -*- coding: utf-8 -*-
"""High-Density Sweep & Calibration Runner for aVolClusterPOI on NQ.

Targets higher zone frequency (>= 1 zone per hour -> 15-30 zones per session)
testing both Time Bars (1m) and Tick Bars (120t, 300t, 600t).

Evaluates:
- Event density per session (target: 15-30 zones/session)
- Session coverage (target: >= 85% of sessions)
- Cluster width distribution in ticks (target: 4-24 ticks / 1-6 pts)
- OFF_PRICE vs AT_PRICE ratio
- Chronological session persistence
"""
...
def run_avol_for_contract_dense(
    pq_path: Path,
    contract: str,
    ...
    """Run high-density grid across multiple bar resolutions on a single contract."""
    ...
    # Process each bar type (e.g. time_1m, tick_120, tick_300, tick_600)
    for b_type, b_info in bar_series_types.items():
        ...
        # Filter configs belonging to this bar type
...
def main():
    ...
    parser.add_argument("--output-json", type=Path, default=Path(r"docs/research/avolcluster_nq_dense_sweep_result.json"))
    ...
    print(f"=== aVolClusterPOI High-Density Sweep (>= 1 zone/hour) on NQ ===")
    ...
    bar_series_types = {
        ...
        "tick_300": {"kind": "tick", "param": 300},
        "tick_600": {"kind": "tick", "param": 600},
    }
    ...
    for b_type, b_info in bar_series_types.items():
        if b_type == "time_1m":
            w_opts = [3, 5, 10]
        elif b_type == "tick_120":
            w_opts = [5, 10, 15]
        elif b_type == "tick_300":
            w_opts = [3, 5, 10]
        else: # tick_600
            w_opts = [3, 5, 8]
    ...
    for contract in contracts:
        ...
        c_events = run_avol_for_contract_dense(pq_path, contract, v_sess, bar_series_types, grid_configs, profiles)
    ...
    print(f"\nProcessing complete in {elapsed/60:.1f} minutes. Compiling high-density ranking...")

    # Ranking targeting 15 - 30 zones per session (>= 1 zone/hour)
    summary_table = []
    for cfg in grid_configs:
        ...
        # Fitness scoring: target 15-25 zones/session (~1 zone/hour), coverage >= 90%, width 4-20 ticks
        target_dens = 20.0
    ...
    out_payload = {
        "schema": "avolcluster_nq_dense_sweep_v1",
        "instrument": "NQ",
        "generated_utc": datetime.now(timezone.utc).isoformat(),
        "total_sessions": total_valid_sessions,
        "elapsed_seconds": round(elapsed, 1),
        "target_density_note": "Targeting >= 1 zone per hour (15-25 zones per session)",
    ...
    print(f"\n{'='*85}")
    print(f"HIGH-DENSITY SWEEP COMPLETE — Results written to: {args.output_json}")
    print(f"{'='*85}")
    print(f"{'Rank':<5} {'Config ID':<30} {'Total OFF':<10} {'Zones/Sess':<12} {'Zones/Hour':<12} {'Coverage':<10} {'Fitness':<8}")
    print(f"{'-'*85}")
    for i, row in enumerate(summary_table[:15], 1):
        print(f"{i:<5} {row['cfg_id']:<30} {row['total_off_price']:<10} {row['zones_per_session_mean']:<12} {row['zones_per_hour_mean']:<12} {row['session_coverage_pct']}%{'':<3} {row['fitness_score']:<8}")
    print(f"{'='*85}")
