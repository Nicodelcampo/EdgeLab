...
def finalize(root,out,replications):
 ...
 secondary=aggregate(rows,"seconds",CLOCK_H,replications); result={"schema":"bt2a_gate2_p2a_result_v1","status":"COMPLETE_P2A_POST_OUTCOME_DIAGNOSTIC","n_sessions":len(rows),"primary_family":primary,"secondary_clock_family":secondary,"multiplicity":"HOLM_OVER_16_PRIMARY_CELLS; SECONDARY_CLOCK_UNADJUSTED_DESCRIPTIVE","unit":"CME_SESSION","timeout_in_estimand":True,"CAMPAIGN_OUTCOMES_OPENED":True,"EDGE_DECLARED":False,"confirmatory_eligible":False,"promotion_eligible":False}; result["payload_sha256"]=canonical(result); atomic(Path(out)/"gate2_p2a_result.json",result); return result
def preflight(root,event_store):
 registry,_,_=_context(root); missing=[i for i in range(len(registry["sessions"])) if not (Path(event_store)/"checkpoints"/f"session_{i:03d}.json").is_file()]; path=Path(root)/"specs"/"bt2a_gate2_first_passage_v1.json"; status=json.loads(path.read_text()).get("status") if path.is_file() else "MISSING"; return {"status":"PASS_READY_FOR_AUTHORIZATION" if not missing and str(status).startswith("FROZEN") else "NOT_READY","spec_status":status,"n_expected_sessions":len(registry["sessions"]),"n_missing_event_checkpoints":len(missing),"missing_event_checkpoint_indices":missing[:50],"outcomes_opened":False}
def main():
 ...
 result=finalize(root,a.output_dir,a.bootstrap_replications) if a.finalize else run_session(root,a.data_dir,a.event_store_dir,a.output_dir,a.session_index,a.control_replications); print(json.dumps({k:v for k,v in result.items() if k not in {"cells","primary_family","secondary_clock_family"}},indent=2,sort_keys=True)); return 0
