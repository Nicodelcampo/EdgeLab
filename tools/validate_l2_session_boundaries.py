...
1. INVERSION DE RELOJ: `last_ts_us(D)` tiene que ser <= `first_ts_us(D+1)`. Si no, HARD FAIL
   (`continuity_status=FAIL_CLOCK_INVERSION`) -- esto nunca es aceptable, sea cual sea el gap.
2. CLASIFICACION DEL GAP (HEURISTICA, no un calendario CME certificado -- para eso existe
   ...
   gap del orden de decenas de horas. Cualquier gap que no calce con ninguno de los dos
...
def classify_gap(day_before: date, day_after: date, gap_seconds: float) -> str:
    ...
    if calendar_days == 1 and dow_before in _CONTINUOUS_DOW and gap_seconds <= CONTINUOUS_GAP_MAX_S:
