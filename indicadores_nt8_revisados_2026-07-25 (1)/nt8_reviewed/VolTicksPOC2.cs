			...
			double threshold, int windowCount, long zoneId, int touchCount, string reason)
		{
			...
			try
			{
				if (writer == null)
				{
					...
					writer.WriteLine("# meta,indicator=VolTicksPOC2,version=2.0,instrument="
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private VolTicksPOC2[] cacheVolTicksPOC2;
		public VolTicksPOC2 VolTicksPOC2(int avgPeriod, int ratioWindowBars, double detectionPercentile, int minRatioSamples, double exportFloorPercentile, int priceMarkTicks, VTP2InvalidationMode invalidationMode, int maxAgeBars, int maxTouches, string eventLogPath, int opacity, int visualExtendBars, int maxRenderedZones)
		{
			return VolTicksPOC2(Input, avgPeriod, ratioWindowBars, detectionPercentile, minRatioSamples, exportFloorPercentile, priceMarkTicks, invalidationMode, maxAgeBars, maxTouches, eventLogPath, opacity, visualExtendBars, maxRenderedZones);
		}

		public VolTicksPOC2 VolTicksPOC2(ISeries<double> input, int avgPeriod, int ratioWindowBars, double detectionPercentile, int minRatioSamples, double exportFloorPercentile, int priceMarkTicks, VTP2InvalidationMode invalidationMode, int maxAgeBars, int maxTouches, string eventLogPath, int opacity, int visualExtendBars, int maxRenderedZones)
		{
			if (cacheVolTicksPOC2 != null)
				for (int idx = 0; idx < cacheVolTicksPOC2.Length; idx++)
					if (cacheVolTicksPOC2[idx] != null && cacheVolTicksPOC2[idx].AvgPeriod == avgPeriod && cacheVolTicksPOC2[idx].RatioWindowBars == ratioWindowBars && cacheVolTicksPOC2[idx].DetectionPercentile == detectionPercentile && cacheVolTicksPOC2[idx].MinRatioSamples == minRatioSamples && cacheVolTicksPOC2[idx].ExportFloorPercentile == exportFloorPercentile && cacheVolTicksPOC2[idx].PriceMarkTicks == priceMarkTicks && cacheVolTicksPOC2[idx].InvalidationMode == invalidationMode && cacheVolTicksPOC2[idx].MaxAgeBars == maxAgeBars && cacheVolTicksPOC2[idx].MaxTouches == maxTouches && cacheVolTicksPOC2[idx].EventLogPath == eventLogPath && cacheVolTicksPOC2[idx].Opacity == opacity && cacheVolTicksPOC2[idx].VisualExtendBars == visualExtendBars && cacheVolTicksPOC2[idx].MaxRenderedZones == maxRenderedZones && cacheVolTicksPOC2[idx].EqualsInput(input))
						return cacheVolTicksPOC2[idx];
			return CacheIndicator<VolTicksPOC2>(new VolTicksPOC2(){ AvgPeriod = avgPeriod, RatioWindowBars = ratioWindowBars, DetectionPercentile = detectionPercentile, MinRatioSamples = minRatioSamples, ExportFloorPercentile = exportFloorPercentile, PriceMarkTicks = priceMarkTicks, InvalidationMode = invalidationMode, MaxAgeBars = maxAgeBars, MaxTouches = maxTouches, EventLogPath = eventLogPath, Opacity = opacity, VisualExtendBars = visualExtendBars, MaxRenderedZones = maxRenderedZones }, input, ref cacheVolTicksPOC2);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.VolTicksPOC2 VolTicksPOC2(int avgPeriod, int ratioWindowBars, double detectionPercentile, int minRatioSamples, double exportFloorPercentile, int priceMarkTicks, VTP2InvalidationMode invalidationMode, int maxAgeBars, int maxTouches, string eventLogPath, int opacity, int visualExtendBars, int maxRenderedZones)
		{
			return indicator.VolTicksPOC2(Input, avgPeriod, ratioWindowBars, detectionPercentile, minRatioSamples, exportFloorPercentile, priceMarkTicks, invalidationMode, maxAgeBars, maxTouches, eventLogPath, opacity, visualExtendBars, maxRenderedZones);
		}

		public Indicators.VolTicksPOC2 VolTicksPOC2(ISeries<double> input , int avgPeriod, int ratioWindowBars, double detectionPercentile, int minRatioSamples, double exportFloorPercentile, int priceMarkTicks, VTP2InvalidationMode invalidationMode, int maxAgeBars, int maxTouches, string eventLogPath, int opacity, int visualExtendBars, int maxRenderedZones)
		{
			return indicator.VolTicksPOC2(input, avgPeriod, ratioWindowBars, detectionPercentile, minRatioSamples, exportFloorPercentile, priceMarkTicks, invalidationMode, maxAgeBars, maxTouches, eventLogPath, opacity, visualExtendBars, maxRenderedZones);
		}
	}
